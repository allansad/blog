...
개발자 도구를 열어 버튼을 살펴보면, `<a data-locale="en-US">`에 원하는 속성값이 있는 것을 확인할 수 있습니다.
...
addEventListener로 keydown이벤트를 등록하고,window.location.href`를 사용하여 영어 페이지로 이동하도록 만들었습니다.
...
@match를 `en-US`로 바꾸고 링크가 담긴 태그를 `a[data-locale="ko"]`로 변경시키면 완성입니다.
...
특히 개발을 막 시작한 입장에서 배운 것을 실제로 활용해 나만의 기능을 만들 수 있다는 점이 더욱 매력적으로 다가왔습니다.
